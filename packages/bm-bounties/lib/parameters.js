...
// Add "after" and "before" properties to terms which can be used to limit bounties in time.
function addTimeParameter (parameters, terms, apolloClient) {

  // console.log("// addTimeParameter")

  if (typeof parameters.selector.BountiesedAt === "undefined") {
...
addCallback("bounties.parameters", addTimeParameter);

// limit the number of items that can be requested at once
function limitBounties (parameters, terms, apolloClient) {
  var maxLimit = 200;
...
addCallback("bounties.parameters", limitBounties);
